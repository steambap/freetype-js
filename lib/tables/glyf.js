...
function loadComplexGlyph (reader) {
	...
	do {
		...
		const subglyph = {xx: 1, xy: 0, yx: 0, yy: 0, tx: 0, ty: 0, p1: -1, p2: -1};
		subglyph.glyphIndex = reader.readUInt16BE();
		// read arguments
		...
		if (flags & WE_HAVE_A_SCALE) {
			subglyph.xx = subglyph.yy = reader.readF2dot14();
		} else if (flags & WE_HAVE_AN_X_AND_Y_SCALE) {
			subglyph.xx = reader.readF2dot14();
			subglyph.yy = reader.readF2dot14();
		} else if (flags & WE_HAVE_A_TWO_BY_TWO) {
			subglyph.xx = reader.readF2dot14();
			subglyph.yx = reader.readF2dot14();
			subglyph.xy = reader.readF2dot14();
			subglyph.yy = reader.readF2dot14();
		}
